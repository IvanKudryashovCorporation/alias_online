...
def _resolve_room_player_by_client_id(connection, room_code, client_id):
    clean_client_id = _normalize_client_id(client_id)
    ...
    row = connection.execute(
        ...
        LIMIT 1
        """,
        (room_code, clean_client_id),
    ).fetchone()
    if row is None:
        return ""
    return (row["player_name"] or "").strip()


def _resolve_room_player_identity(connection, room_code, player_name="", client_id=""):
    clean_player_name = (player_name or "").strip()
    resolved_by_client = _resolve_room_player_by_client_id(connection, room_code, client_id)
    if resolved_by_client:
        if clean_player_name:
            resolved_by_name = _resolve_room_player_name(connection, room_code, clean_player_name)
            if resolved_by_name and _same_player_name(resolved_by_name, resolved_by_client):
                return resolved_by_name
        return resolved_by_client

    if clean_player_name:
        return _resolve_room_player_name(connection, room_code, clean_player_name)
    return ""
...
def _process_room_guess(connection, room_code, player_name, guess, client_id=""):
    room = connection.execute("SELECT * FROM rooms WHERE code = ?", (room_code,)).fetchone()
    if room is None:
        raise ValueError("Room not found.")
    resolved_player_name = _resolve_room_player_identity(connection, room_code, player_name, client_id)
...
class RoomHandler(BaseHTTPRequestHandler):
    ...
    def _handle_room_state(self, room_code, query):
        ...
        client_id = _normalize_client_id((params.get("client_id", [""])[0] or "").strip())
        since_id = None
    ...
    def _handle_room_guess(self, room_code):
        ...
        with _connect() as connection:
            try:
                result = _process_room_guess(connection, room_code, player_name, guess, client_id=client_id)
            except ValueError as error:
                message = str(error)
    ...
    def _handle_start_game(self, room_code):
        ...
        self._json_response(
            200,
            {
                ...
                "explainer_mic_muted": updated.get("explainer_mic_muted", True) if updated else True,
                "explainer_mic_state": updated.get("explainer_mic_state", "idle") if updated else "idle",
                "can_see_word": updated.get("can_see_word", False) if updated else False,
