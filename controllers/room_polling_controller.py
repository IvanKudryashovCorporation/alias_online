...
class RoomPollingController:
    ...
    def __init__(self, screen):
        ...
        self._rejoin_lock = Lock()  # Protects rejoin state
        self._poll_in_flight = False
        self._poll_token = 0
        self._poll_started_at = 0.0
        self._poll_version = ""  # Version of last applied poll
        self._poll_current_interval = config.POLLING_INTERVAL_SECONDS  # Current adaptive interval
        self._poll_consecutive_errors = 0  # Track errors for backoff
    ...
    def _apply_poll_backoff(self):
        """Increase polling interval for error backoff."""
        self._poll_consecutive_errors += 1
        new_interval = config.POLLING_INTERVAL_SECONDS * (config.POLLING_ERROR_BACKOFF_FACTOR ** self._poll_consecutive_errors)
        self._poll_current_interval = min(new_interval, config.POLLING_MAX_BACKOFF_SECONDS)
        logger.warning(f"Poll error #{self._poll_consecutive_errors}, backoff interval to {self._poll_current_interval:.2f}s")
        # Reschedule poll with new interval
    ...
    def _reset_poll_backoff(self):
        ...
        if self._poll_consecutive_errors > 0:
            logger.info(f"Poll success, resetting backoff from {self._poll_current_interval:.2f}s to {config.POLLING_INTERVAL_SECONDS:.2f}s")
            self._poll_consecutive_errors = 0
            self._poll_current_interval = config.POLLING_INTERVAL_SECONDS
            self._reschedule_polling()
    ...
    def request_rejoin_state(self):
        ...
        logger.debug(f"Requesting rejoin for room {room_code} as {player_name} (token={request_token})")
        Thread(
    ...
    def _rejoin_worker(self, request_token, room_code, player_name, is_guest, client_id):
        ...
        try:
            ...
            logger.warning(f"Rejoin connection error: {error}")
            payload = {"token": request_token, "status": "connection_error", "message": str(error)}
        except ValueError as error:
            logger.warning(f"Rejoin validation error: {error}")
            payload = {"token": request_token, "status": "value_error", "message": str(error)}
        except Exception as error:
            logger.error(f"Rejoin unexpected error: {error}", exc_info=True)
            payload = {"token": request_token, "status": "error", "message": str(error)}
    ...
    def _finish_rejoin_state(self, payload):
        ...
        with self._rejoin_lock:
            if token != self._rejoin_request_token:
                logger.debug(f"Rejoin token mismatch: {token} vs {self._rejoin_request_token}, ignoring")
                return
            self._rejoin_in_flight = False

        if self.screen.manager is None or self.screen.manager.current != self.screen.name:
            logger.debug("Screen not active, discarding rejoin response")
            return

        status = payload.get("status")
        if status != "success":
            message = payload.get("message") or "Не удалось синхронизировать комнату."
            logger.warning(f"Rejoin failed with status={status}: {message}")
        ...
        if isinstance(server_state, dict):
            ...
            logger.debug(
                f"Rejoin version check: incoming={incoming_version}({incoming_phase}), "
                f"current={current_version}({current_phase}), priority={incoming_phase_priority} vs {current_phase_priority}, "
            ...
            if will_apply:
                ...
                if (self.screen._countdown_event is not None and
                    ...
                    logger.info("Rejoin blocked: countdown timer active, don't skip to round yet")
                    return
                ...
                logger.debug("Rejoin skipped: rejoin state is not newer or phase downgraded. Keeping current state.")
                self._rejoin_recover_attempts = 0
                if app is not None:
                    app.set_active_room(initial_state.get("room", joined_room))
    ...
    def _poll_state(self):
        ...
        with self._poll_lock:
            if self._poll_in_flight:
                if self._poll_started_at > 0 and (time.time() - self._poll_started_at) > 9.0:
                    logger.warning("Poll request timeout (9s), marking as complete")
                    self._poll_in_flight = False
                    self._poll_started_at = 0.0
                else:
                    return
        ...
        logger.debug(f"Starting poll for room {room_code} (token={request_token}, since_id={since_id})")
        Thread(
    ...
    def _poll_state_worker(self, request_token, room_code, player_name, client_id, since_id):
        ...
        try:
            ...
            logger.warning(f"Poll connection error: {error}")
            result["status"] = "connection_error"
            result["message"] = str(error)
        except ValueError as error:
            logger.warning(f"Poll validation error: {error}")
            result["status"] = "value_error"
            result["message"] = str(error)
        except Exception as error:
            logger.error(f"Poll unexpected error: {error}", exc_info=True)
            result["status"] = "error"
            result["message"] = str(error)
    ...
    def _finish_poll_state(self, payload):
        ...
        with self._poll_lock:
            if token != self._poll_token:
                logger.debug(f"Poll token mismatch: {token} vs {self._poll_token}, ignoring stale response")
                return
            self._poll_in_flight = False
            self._poll_started_at = 0.0

        if self.screen.manager is None or self.screen.manager.current != self.screen.name:
            logger.debug("Screen not active, discarding poll response")
            return

        logger.debug("Poll response received and processed")

        if (payload.get("room_code") or "").strip().upper() != (self.screen.room_code or "").strip().upper():
            logger.warning(f"Poll room code mismatch: {payload.get('room_code')} vs {self.screen.room_code}")
            return
        ...
        if status == "success":
            ...
            logger.debug(
                f"Poll success. Phase: {incoming_phase}, Version: {incoming_version}, "
                f"Current version: {self.screen._room_state_version}"
            )
            ...
            if not bool(viewer_state.get("is_player")):
                if self._rejoin_recover_attempts < 2:
                    ...
                    logger.info(f"Player not in room, requesting rejoin (attempt {self._rejoin_recover_attempts})")
                    self.screen.status_label.color = self.screen.COLORS["warning"]
                ...
                logger.warning("Player not in room after max rejoin attempts, navigating away")
                app = App.get_running_app()
                if app is not None:
            ...
            if will_apply:
                ...
                if (self.screen._countdown_event is not None and
                    incoming_phase == "round" and current_phase == "countdown"):
                    logger.info("Poll blocked: countdown timer active, don't skip to round yet")
                    self.screen._ensure_interaction_ready()
                    self._reset_poll_backoff()  # Success, reset backoff even though we didn't apply
                    return

                logger.info(
                    f"Poll applying new state. Phase changing from {self.screen._current_phase()} "
                    f"to {state.get('game_phase', '?')}"
                )
                ...
                if room_version_check != incoming_version:
                    logger.warning(
                        f"State version mismatch! incoming={incoming_version}, room.updated_at={room_version_check}"
                    )
                ...
                current_phase = self.screen._current_phase()
                logger.debug(f"Poll skipped: version too old. Current phase stays: {current_phase}")
